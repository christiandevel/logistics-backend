...
export class PostgresAuthRepository implements AuthRepository {
	...
	async setInitialPassword(userId: string, newPassword: string): Promise<void> {
		const query = `
			...
			WHERE id = $2 AND requires_password_change = true
			RETURNING id, email, requires_password_change;
		`
		
		const result = await this.pool.query(query, [newPassword, userId]);
		if (result.rows.length === 0) {
			throw new Error("User not found or password change not required");
		}
		
		console.log('Initial password set for user:', {
			userId: result.rows[0].id,
			email: result.rows[0].email,
			requiresPasswordChange: result.rows[0].requires_password_change
		});
	}
